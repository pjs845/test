...
#col.insert_one({"_id":13, "name": "하길동", "addr": "서천", "rdate":dt_now})

#docs = col.find().limit(5)
#docs = col.find().sort("_id").limit(5)
#docs = col.find().sort("_id", -1).limit(5)
#docs = col.find().sort("name").limit(5)
#docs = col.find().sort("name", -1).limit(5)
docs = col.find().sort("rdate", -1).limit(5)
for doc in docs:
    print(doc)
